...
mod tests {
    ...
    #[test]
    fn test_escaped_module_symbol_names() {
        with_test_lock(|| {
            let grammar = execute_fixture(
                &[(
                    "grammar.mjs",
                    r"
                    export const root = rule(() => seq(for_, value__, alias('x', label_))),
                      for_ = rule(() => 'for'), value__ = rule(() => 'value'), label_ = rule();
                    export default {name: 'escaped', start: root, word: for_};
                    ",
                )],
                "grammar.mjs",
            );
            assert_eq!(grammar["word"], "for");
            assert_eq!(grammar["rules"]["root"]["members"][0]["name"], "for");
            assert_eq!(grammar["rules"]["root"]["members"][1]["name"], "value_");
            assert_eq!(grammar["rules"]["root"]["members"][2]["value"], "label");
            assert!(grammar["rules"].get("for_").is_none());
        });
    }
    ...
    #[test]
    fn test_module_explicit_body_and_declaration_order() {
        with_test_lock(|| {
            let grammar = execute_fixture(
                &[
                    (
                    ...
                    (
                        ...
                        export const z_keyword = rule(function () {
                          if (arguments.length) throw new Error('unexpected builder arguments');
                          const changed = base.z_keyword.body();
                          changed.value = 'bool';
                          return choice(changed, base.z_keyword.body());
                        });
    ...
    fn test_module_body_cache_and_exact_definition() {
        with_test_lock(|| {
            let grammar = execute_fixture(
                &[
                    (
                        ...
                        export const item = rule(() => {
                          if (++calls > 1) throw new Error('evaluated twice');
                          return seq('base', item);
                        }), broken = rule(() => {
                          if (++failures > 1) throw new Error('failure evaluated twice');
                    ...
                    (
                        ...
                        r"
                        import * as base from './base.mjs';
                        export const item = rule(() => seq(base.item.body(), 'middle'));
                    ...
                    (
                        ...
                        export const item = rule(() => 'leaf'),
                          broken = rule(() => 'recovered'),
                          root = rule(function () {
                            ...
                            try { base.broken.body(); } catch (error) { second = error; }
                            if (!first || !first.message.includes('expected failure') || first !== second)
                              throw new Error('failure not cached');
                            const changed = base.item.body();
                            changed.members[0].value = 'changed';
                            return seq(changed, base.item.body(), middle.item.body(), base.item);
                          });
            ...
            assert_eq!(
                grammar["rules"]["root"],
                serde_json::json!({"type": "SEQ", "members": [
                    {"type": "SEQ", "members": [
                        {"type": "STRING", "value": "changed"},
                        {"type": "SYMBOL", "name": "item"}
                    ]},
                    base,
                    {"type": "SEQ", "members": [base, {"type": "STRING", "value": "middle"}]},
                    {"type": "SYMBOL", "name": "item"}
                ]})
            );
            assert_eq!(
                grammar["rules"]["item"],
                serde_json::json!({"type": "STRING", "value": "leaf"})
            );
        });
    }
    ...
    fn test_module_grammar_errors() {
        with_test_lock(|| {
            for (source, expected) in [
                ...
                (
                    "export const root = rule(); export default { name: 'test', start: root, externals: [root] };",
                    "The start rule must have a body",
                ),
                (
                    "export const root = rule(() => 'x'); root.body(); export default {name: 'test', start: root};",
                    "Rule bodies can only be accessed during grammar evaluation",
                ),
                (
                    "export const root = rule(() => root.body()); export default {name: 'test', start: root};",
                    "Recursive .body() evaluation",
                ),
                (
                    "export const root = rule(() => other.body()), other = rule(() => root.body()); export default {name: 'test', start: root};",
                    "Recursive .body() evaluation",
                ),
                (
                    "export const root = rule(() => label.body()), label = rule(); export default {name: 'test', start: root};",
                    "Rule 'label' has no body",
                ),
                (
                    "export const root = rule(() => ext.body()), ext = rule(); export default {name: 'test', start: root, externals: [ext]};",
                    "Rule 'ext' has no body",
                ),
            ] {
                let temp_dir = TempDir::new().unwrap();
