// Legacy callback/proxy grammar API and the expression constructors shared by
// both grammar formats. This file is evaluated before dsl.js.
function alias(rule, value) {
  ...
  if (ruleDefinitions.has(value)) {
    // Module rule names are assigned after all exports have been collected.
    return { ...result, named: true, value };
  }
...
function normalize(value) {
  ...
  if (typeof value === "function") {
    throw new TypeError("Expected a rule handle or expression, not a function. Call helpers to construct expressions; use rule(() => ...) to declare rules.");
  }
  if (value === null) {
    throw new TypeError("Invalid rule: null");
  }
