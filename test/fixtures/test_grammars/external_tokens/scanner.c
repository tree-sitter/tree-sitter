...
enum { percent_string, percent_string_start, percent_string_end };
...
void *tree_sitter_external_tokens_external_scanner_create() {
  ...
  *scanner = (Scanner){ .open_delimiter = 0, .close_delimiter = 0, .depth = 0 };
  return scanner;
}
...
unsigned tree_sitter_external_tokens_external_scanner_serialize(void *payload, char *buffer) {
  return true;
}
