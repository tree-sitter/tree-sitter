  ...
  matches(node, startPosition, endPosition, options) {
    ...
    for (let i = 0; i < rawCount; i++) {
      ...
      if (this.textPredicates[pattern].every(p => p(captures))) {
        ...
        result[filteredIndex] = { pattern, captures };
        const setProperties = this.setProperties[pattern];
        if (setProperties) result[filteredIndex].setProperties = setProperties;
        const assertedProperties = this.assertedProperties[pattern];
        if (assertedProperties) result[filteredIndex].assertedProperties = assertedProperties;
        const refutedProperties = this.refutedProperties[pattern];
        if (refutedProperties) result[filteredIndex].refutedProperties = refutedProperties;
      }
    }
