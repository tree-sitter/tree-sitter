...
describe("Node", () => {
  ...
  describe(".child(), .firstChild, .lastChild, firstChildForIndex()", () => {
    it("returns null when the node has no children", () => {
      ...
      assert.equal(variableNode.firstChildForIndex(1), null);
      assert.equal(variableNode.firstNamedChildForIndex(1), null);
    });
  });
