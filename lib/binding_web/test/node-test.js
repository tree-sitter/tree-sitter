...
describe('Node', () => {
  ...
  describe('.nextSibling and .previousSibling', () => {
    it("returns the node's next and previous sibling", () => {
      ...
      assert.equal(sumNode.children[0].id, sumNode.children[1].previousSibling.id);
  ...
  describe('.nextNamedSibling and .previousNamedSibling', () => {
    it("returns the node's next and previous named sibling", () => {
      tree = parser.parse('x10 + 1000');
      const sumNode = tree.rootNode.firstChild.firstChild;
      assert.equal(sumNode.namedChildren[1].id, sumNode.namedChildren[0].nextNamedSibling.id);
      assert.equal(sumNode.namedChildren[0].id, sumNode.namedChildren[1].previousNamedSibling.id);
    });
  });
  ...
  describe('.descendantForPosition(min, max)', () => {
    it('returns the smallest node that spans the given range', () => {
      ...
      assert.equal('identifier', sumNode.descendantForPosition({ row: 0, column: 1 }, { row: 0, column: 2 }).type);

      assert.equal('+', sumNode.descendantForPosition({ row: 0, column: 4 }).type);
  ...
  describe('.namedDescendantForPosition(min, max)', () => {
    it('returns the smallest named node that spans the given range', () => {
      ...
      assert.equal(sumNode.namedDescendantForPosition({ row: 0, column: 1 }, { row: 0, column: 2 }).type, 'identifier');

      assert.equal(sumNode.namedDescendantForPosition({ row: 0, column: 4 }).type, 'binary_expression');
    });
  });
  ...
  describe('.descendantsOfType(type, min, max)', () => {
    it('finds all of the descendants of the given type in the given range', () => {
      ...
      descendants = outerSum.descendantsOfType(['identifier', 'number'], { row: 0, column: 0 }, { row: 0, column: 30 });
      assert.deepEqual(
        descendants.map(node => node.startIndex),
        [0, 4, 8, 12, 16, 20]
      );
