...
static inline bool ts_tree_cursor_is_entry_visible(const TreeCursor *self, uint32_t index) {
  ...
  if (index == 0 || ts_subtree_visible(*entry->subtree)) {
    ...
    return ts_language_alias_at(
      self->tree->language, parent_entry->subtree->ptr->production_id, entry->structural_child_index
    );
  } else {
    return false;
  }
...
static inline CursorChildIterator ts_tree_cursor_iterate_children(const TreeCursor *self) {
  ...
  const TSSymbol *alias_sequence
    = ts_language_alias_sequence(self->tree->language, last_entry->subtree->ptr->production_id);
...
TreeCursorStep ts_tree_cursor_goto_sibling_internal(
  TSTreeCursor *_self, bool (*advance)(CursorChildIterator *, TreeCursorEntry *, bool *)
) {
  TreeCursor *self = (TreeCursor *)_self;
  uint32_t initial_size = self->stack.size;
...
TreeCursorStep ts_tree_cursor_goto_previous_sibling_internal(TSTreeCursor *_self) {
  ...
  TreeCursorStep step
    = ts_tree_cursor_goto_sibling_internal(_self, ts_tree_cursor_child_iterator_previous);
...
void ts_tree_cursor_goto_descendant(TSTreeCursor *_self, uint32_t goal_descendant_index) {
  ...
  for (;;) {
    ...
    uint32_t next_descendant_index = entry->descendant_index
...
void ts_tree_cursor_current_status(
  ...
  *can_have_later_siblings_with_this_field = false;
  ...
  for (unsigned i = self->stack.size - 1; i > 0; i--) {
    ...
    const TSSymbol *alias_sequence
      = ts_language_alias_sequence(self->tree->language, parent_entry->subtree->ptr->production_id);

#define subtree_symbol(subtree, structural_child_index) \
  ((!ts_subtree_extra(subtree) && alias_sequence && alias_sequence[structural_child_index]) \
     ? alias_sequence[structural_child_index] \
     : ts_subtree_symbol(subtree))

    // Stop walking up when a visible ancestor is found.
    TSSymbol entry_symbol = subtree_symbol(*entry->subtree, entry->structural_child_index);
    ...
    if (!*has_later_siblings) {
      ...
      for (unsigned j = entry->child_index + 1; j < sibling_count; j++) {
        ...
        TSSymbolMetadata sibling_metadata = ts_language_symbol_metadata(
          self->tree->language, subtree_symbol(sibling, structural_child_index)
        );
...
#undef subtree_symbol
    ...
    if (!ts_subtree_extra(*entry->subtree)) {
      ...
      ts_language_field_map(
        self->tree->language, parent_entry->subtree->ptr->production_id, &field_map, &field_map_end
      );
