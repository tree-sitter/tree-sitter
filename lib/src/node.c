...
static inline TSNode ts_node__prev_sibling(TSNode self, bool include_anonymous) {
  ...
  while (!ts_node_is_null(node)) {
    ...
    while (ts_node_child_iterator_next(&iterator, &child)) {
      ...
      if (iterator.position.bytes == target_end_byte && (!self_is_empty || ts_subtree_has_trailing_empty_descendant(ts_node__subtree(child), self_subtree))) {
        found_child_containing_target = true;
        break;
      }
...
static inline TSNode ts_node__first_child_for_byte(
  TSNode self, uint32_t goal, bool include_anonymous
) {
  TSNode node = self;
  bool did_descend = true;
...
static inline TSNode ts_node__descendant_for_byte_range(
  TSNode self, uint32_t range_start, uint32_t range_end, bool include_anonymous
) {
  TSNode node = self;
...
static inline TSNode ts_node__descendant_for_point_range(
  TSNode self, TSPoint range_start, TSPoint range_end, bool include_anonymous
) {
  TSNode node = self;
  TSNode last_visible_node = self;
...
TSNode ts_node_parent(TSNode self) {
  ...
  while (did_descend) {
    ...
    NodeChildIterator iterator = ts_node_iterate_children(&node);
    while (ts_node_child_iterator_next(&iterator, &child)) {
      if (ts_node_start_byte(child) > ts_node_start_byte(self) || child.id == self.id)
        break;
