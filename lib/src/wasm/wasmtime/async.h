 ...
 *
 * As for asynchronous host calls - the reverse contract is upheld. Wasmtime will keep all
 * parameters to the function alive and unmodified until the
 ...
 * Instances created and functions called with this Config must be called through their asynchronous
 * APIs, however. For example using wasmtime_func_call will panic when used with this config.
 *
 ...
 *
 * The amount of stack space guaranteed for host functions is async_stack_size - max_wasm_stack, so
 * take care not to set these two values close to one another; doing so may cause host functions to
 ...
 *        disable yielding.
 */
WASM_API_EXTERN void wasmtime_context_fuel_async_yield_interval(
  wasmtime_context_t *context, uint64_t interval
);

/**
 * \brief Configures epoch-deadline expiration to yield to the async caller and the update the
 * deadline.
 *
...
typedef void (*wasmtime_func_async_callback_t)(
  ...
  wasmtime_val_t *results, size_t nresults, wasm_trap_t **trap_ret,
 ...
 *
 * Returns true if the function call has completed. After this function returns true, it should
 * *not* be called again for a given future.
 *
 ...
 * (see wasmtime_context_epoch_deadline_async_yield_and_update). The function may also return false
 * if asynchronous host functions have been called, which then calling this  function will call the
 ...
 * must be polled using #wasmtime_call_future_poll, and is owned and must be deleted using
 * #wasmtime_call_future_delete.
 *
 * The `trap_ret` and `error_ret` pointers may *not* be `NULL` and the returned memory is owned by
 * the caller.
 *
 * All arguments to this function must outlive the returned future and be unmodified until the
 * future is deleted.
 */
...
wasmtime_config_host_stack_creator_set(wasm_config_t *, wasmtime_stack_creator_t *);
