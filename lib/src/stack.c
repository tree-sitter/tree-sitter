...
static StackNode *stack_node_new(
  StackNode *previous_node, Subtree subtree, bool is_pending, TSStateId state, StackNodeArray *pool
) {
...
static StackVersion ts_stack__add_version(
  Stack *self, StackVersion original_version, StackNode *node
) {
  StackHead head = {
    .node = node,
...
static void ts_stack__add_slice(
  Stack *self, StackVersion original_version, StackNode *node, SubtreeArray *subtrees
) {
...
static StackSliceArray stack__iter(
  Stack *self, StackVersion version, StackCallback callback, void *payload, int goal_subtree_count
) {
  ...
  if (goal_subtree_count >= 0) {
    ...
    array_reserve(
      &new_iterator.subtrees, (uint32_t)ts_subtree_alloc_size(goal_subtree_count) / sizeof(Subtree)
    );
  }
...
void ts_stack_push(
  Stack *self, StackVersion version, Subtree subtree, bool pending, TSStateId state
) {
  StackHead *head = array_get(&self->heads, version);
...
inline StackAction summarize_stack_callback(void *payload, const StackIterator *iterator) {
  ...
  array_push(
    session->summary, ((StackSummaryEntry){
...
bool ts_stack_can_merge(Stack *self, StackVersion version1, StackVersion version2) {
  ...
  return head1->status == StackStatusActive && head2->status == StackStatusActive
         ...
         && ts_subtree_external_scanner_state_eq(
           head1->last_external_token, head2->last_external_token
         );
}
...
bool ts_stack_print_dot_graph(Stack *self, const TSLanguage *language, FILE *f) {
  ...
  for (uint32_t i = 0; i < self->heads.size; i++) {
    ...
    fprintf(
      f, "label=%u, fontcolor=blue, weight=10000, labeltooltip=\"node_count: %u\nerror_cost: %u", i,
  ...
  while (!all_iterators_done) {
    ...
    for (uint32_t i = 0; i < self->iterators.size; i++) {
      ...
      fprintf(
        ...
        node->position.extent.row + 1, node->position.extent.column, node->node_count,
      ...
      for (int j = 0; j < node->link_count; j++) {
        ...
        if (!link.subtree.ptr) {
          ...
          fprintf(f, "\"");
          fprintf(
            f, "labeltooltip=\"error_cost: %u\ndynamic_precedence: %u\"",
            ts_subtree_error_cost(link.subtree), ts_subtree_dynamic_precedence(link.subtree)
          );
        }
