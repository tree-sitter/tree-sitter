...
#define LOG(message, character) \
  if (self->logger.log) { \
    snprintf( \
      self->debug_buffer, TREE_SITTER_SERIALIZATION_BUFFER_SIZE, \
...
static void ts_lexer__get_chunk(Lexer *self) {
  ...
  self->chunk = self->input.read(
    self->input.payload, self->current_position.bytes, self->current_position.extent,
    &self->chunk_size
  );
  if (!self->chunk_size) {
...
static void ts_lexer__get_lookahead(Lexer *self) {
  ...
  DecodeFunction decode = self->input.encoding == TSInputEncodingUTF8      ? ts_decode_utf8
...
static void ts_lexer__do_advance(Lexer *self, bool skip) {
  if (self->lookahead_size) {
    if (self->data.lookahead == '\n') {
      ...
      bool is_bom = self->current_position.bytes == 0 && self->data.lookahead == BYTE_ORDER_MARK;
      if (!is_bom)
...
void ts_lexer_finish(Lexer *self, uint32_t *lookahead_end_byte) {
  ...
  if (self->data.lookahead == TS_DECODE_ERROR) {
    current_lookahead_end_byte
      += 4;  // the maximum number of bytes read to identify an invalid code point
  }
