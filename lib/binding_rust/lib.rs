...
/// The type used for node kind identifiers.
/// Values must be interpreted according to the `Language` of the node it belongs to.
/// Use [Language::node_kind_for_id] to get a string representation.
pub type KindId = ffi::TSSymbol;

/// The type used for the field identifiers.
/// Values must be interpreted according to the `Language` of the node it is attached to.
/// Use [Language::field_name_for_id] to get a string representation.
pub type FieldId = NonZeroU16; // This should remain a non zero version of ffi::TSFieldId
...
impl Language {
    ...
    pub fn field_name_for_id(&self, field_id: FieldId) -> Option<&'static str> {
        let ptr = unsafe { ffi::ts_language_field_name_for_id(self.0, field_id.get()) };
        if ptr.is_null() {
            None
        } else {
            Some(unsafe { CStr::from_ptr(ptr) }.to_str().unwrap())
...
impl<'tree> Node<'tree> {
    ...
    pub fn child_by_field_id(&self, field_id: FieldId) -> Option<Self> {
    ...
    pub fn children_by_field_name<'a>(
        ...
        self.language()
            ...
            .map(move |id| self.children_by_field_id(id, cursor))
            .into_iter()
            .flatten()
    }
