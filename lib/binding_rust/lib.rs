...
impl Query {
    ...
    pub fn new(language: &Language, source: &str) -> Result<Self, QueryError> {
        let ptr = Self::new_ptr(language, source)?;
        unsafe { Self::from_raw_parts(ptr, source) }
    }

    /// Constructs a raw [`TSQuery`] pointer without performing extra checks specific to the rust
    /// bindings, like predicate validation and [`Query`] object construction. Useful for using the
    /// rust bindings to perform more general query analysis.
    pub fn new_ptr(language: &Language, source: &str) -> Result<*mut ffi::TSQuery, QueryError> {
