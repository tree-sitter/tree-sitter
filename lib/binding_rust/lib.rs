...
impl From<ffi::TSQuantifier> for CaptureQuantifier {
    fn from(value: ffi::TSQuantifier) -> Self {
        match value {
            ffi::TSQuantifier_TSQuantifierZero => Self::Zero,
            ffi::TSQuantifier_TSQuantifierZeroOrOne => Self::ZeroOrOne,
            ffi::TSQuantifier_TSQuantifierZeroOrMore => Self::ZeroOrMore,
            ffi::TSQuantifier_TSQuantifierOne => Self::One,
            ffi::TSQuantifier_TSQuantifierOneOrMore => Self::OneOrMore,
...
impl Language {
    ...
    pub fn lookahead_iterator(&self, state: u16) -> Option<LookaheadIterator> {
        ...
        (!ptr.is_null()).then_some(unsafe { LookaheadIterator::from_raw(ptr) })
    }
}
...
impl Parser {
    ...
    pub fn language(&self) -> Option<Language> {
        ...
        (!ptr.is_null()).then_some(Language(ptr))
    }
    ...
    pub fn set_included_ranges(&mut self, ranges: &[Range]) -> Result<(), IncludedRangesError> {
        let ts_ranges: Vec<ffi::TSRange> =
            ranges.iter().copied().map(|range| range.into()).collect();
        let result = unsafe {
