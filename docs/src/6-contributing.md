...
page. Similarly, this command also looks for [Binaryen][binaryen] indicated by the `TREE_SITTER_BINARYEN_PATH`
environment variable. This suite of tools can be downloaded from binaryen's [releases][binaryen-releases] page.
Note that any changes to `crates/language/wasm/**` requires rebuilding the tree-sitter Wasm stdlib via
`cargo xtask build-wasm-stdlib`.
