...
> **NOTE**: Do not use any of the array functions or macros that are prefixed with an underscore and have comments saying
...
When you include literal keywords in the externals array (like externals: $ => ['if', 'then', 'else']), those keywords will
...
> - When consuming characters in a loop, always use the `eof` function to check for the end of file
