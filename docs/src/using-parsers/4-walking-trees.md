...
> cursor cannot walk outside this node. Going to the parent or any sibling
> of the root node will always return `false`. This has no unexpected effects if the given
> input node is the actual `root` node of the tree, but is something to keep in mind
> when using cursors constructed with a node that is not the `root` node.
