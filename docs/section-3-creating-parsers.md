...
* **`bool force_serialize_state`** - A boolean that forces the scanner to serialize its state, even when the `scan` function returns `false`. `force_serialize_state` is initialized to `false` before each call to `scan`.
