...
fn update_cargo(version: &Version) -> Result<()> {
    ...
    let mut new_lines = Vec::<String>::with_capacity(old_lines.size_hint().0);
    ...
    while let Some(line) = old_lines.next() {
        ...
        match line {
            "    .wasmtime_c_api_aarch64_android = .{" => {
                ...
                zig_fetch(new_lines.as_mut(), version, "aarch64-android-c-api.tar.xz")?;
            }
            "    .wasmtime_c_api_aarch64_linux = .{" => {
                let (_, _) = (old_lines.next(), old_lines.next());
                zig_fetch(new_lines.as_mut(), version, "aarch64-linux-c-api.tar.xz")?;
            }
            "    .wasmtime_c_api_aarch64_macos = .{" => {
                let (_, _) = (old_lines.next(), old_lines.next());
                zig_fetch(new_lines.as_mut(), version, "aarch64-macos-c-api.tar.xz")?;
            }
            "    .wasmtime_c_api_riscv64gc_linux = .{" => {
                let (_, _) = (old_lines.next(), old_lines.next());
                zig_fetch(new_lines.as_mut(), version, "riscv64gc-linux-c-api.tar.xz")?;
            }
            "    .wasmtime_c_api_s390x_linux = .{" => {
                let (_, _) = (old_lines.next(), old_lines.next());
                zig_fetch(new_lines.as_mut(), version, "s390x-linux-c-api.tar.xz")?;
            }
            "    .wasmtime_c_api_x86_64_android = .{" => {
                let (_, _) = (old_lines.next(), old_lines.next());
                zig_fetch(new_lines.as_mut(), version, "x86_64-android-c-api.tar.xz")?;
            }
            "    .wasmtime_c_api_x86_64_linux = .{" => {
                let (_, _) = (old_lines.next(), old_lines.next());
                zig_fetch(new_lines.as_mut(), version, "x86_64-linux-c-api.tar.xz")?;
            }
            "    .wasmtime_c_api_x86_64_macos = .{" => {
                let (_, _) = (old_lines.next(), old_lines.next());
                zig_fetch(new_lines.as_mut(), version, "x86_64-macos-c-api.tar.xz")?;
            }
            "    .wasmtime_c_api_x86_64_mingw = .{" => {
                let (_, _) = (old_lines.next(), old_lines.next());
                zig_fetch(new_lines.as_mut(), version, "x86_64-mingw-c-api.zip")?;
            }
            "    .wasmtime_c_api_x86_64_musl = .{" => {
                let (_, _) = (old_lines.next(), old_lines.next());
                zig_fetch(new_lines.as_mut(), version, "x86_64-musl-c-api.tar.xz")?;
            }
            "    .wasmtime_c_api_x86_64_windows = .{" => {
                let (_, _) = (old_lines.next(), old_lines.next());
                zig_fetch(new_lines.as_mut(), version, "x86_64-windows-c-api.zip")?;
            }
            _ => {}
        }
    }
